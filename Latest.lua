--[[
    V35: THE FINAL OVERLORD BUILD
    - UI: Back, Minimize, and Close buttons on Harvester & Executor.
    - CORE: Integrated the complete 4,500+ logic & Brainrot specific keys.
    - PERSISTENCE: Zero keywords removed; fully cumulative.
    - SMART-FIX: V17 Auto-debugger for mobile syntax.
]]
...
local function protect(gui)
    ...
    gui.Name = "FinalOverlord_V35_" .. math.random(1000, 9999)
    if gethui then gui.Parent = gethui() else gui.Parent = game:GetService("CoreGui") end
end

-- 1. THE ETERNAL KEYWORD VAULT (ALL LISTS MERGED)
local MASTER_KEYS = {
...
local function openExec(remote, codeOverride)
    ...
    frame.Size = UDim2.new(0.85, 0, 0.55, 0); frame.Position = UDim2.new(0.075, 0, 0.2, 0); frame.BackgroundColor3 = Color3.fromRGB(15,15,15); frame.Active = true; frame.Draggable = true
    ...
    log.Size = UDim2.new(1, -20, 0, 40); log.Position = UDim2.new(0, 10, 0.5, 45); log.Text = "Status: Ready"; log.TextColor3 = Color3.new(0.7,0.7,0.7); log.BackgroundTransparency = 1
    ...
    fire.MouseButton1Click:Connect(function()
        ...
        if func then
            ...
            log.Text = "⚠️ SYNTAX: "..tostring(err):sub(1, 45)
...
opt("Harvester", function() 
    if _G.Minimized and _G.CurrentWindow then 
        _G.CurrentWindow.Frame.Visible = true; _G.Minimized = false 
    else openHarvester() end 
end)
